...
class Music(commands.Cog):
    ...
    async def _play_song(self, ctx, *, track: str):
        ...
        # reset timer
        music_player.timer.cancel()
        music_player.timer = utils.Timer(music_player.timeout_handler)

        if music_player.playlist.loop:
            await ctx.send("Loop is enabled! Use {}loop to disable".format(config.PREFIX))
            return

        song = await music_player.process_song(track)
        ...
        if song.origin == linkutils.Origins.Default:
            ...
            if music_player.current_song is not None and len(music_player.playlist.playque) == 0:
    ...
    async def _loop(self, ctx):
        ...
        if not music_player.playlist.loop:
            ...
            music_player.playlist.loop = False
    ...
    async def _shuffle(self, ctx):
        ...
        music_player.playlist.shuffle()
        await ctx.send("Shuffled queue :twisted_rightwards_arrows:")

        for song in list(music_player.playlist.playque)[:config.MAX_SONG_PRELOAD]:
            asyncio.ensure_future(music_player.preload(song))
    ...
    async def _move(self, ctx, *args):
        ...
        if current_guild.voice_client is None or (
            ...
            return
        try:
            music_player.playlist.move(oldindex - 1, newindex - 1)
        except IndexError:
