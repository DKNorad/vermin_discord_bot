...
class Settings:
    ...
    async def format(self):
        embed = discord.Embed(title="Settings", description=self.guild.name, color=config.EMBED_COLOR)

        embed.set_thumbnail(url=self.guild.icon_url)
        embed.set_footer(
            text="Usage: {}set setting_name value".format(config.PREFIX))
    ...
    async def user_must_be_in_vc(self, setting, value, ctx):
        if value.lower() == "true":
            ...
            await ctx.send("`Error: Value must be True/False`\nUsage: {}set {} True/False".format(config.PREFIX, setting))
            return False
    ...
    async def default_volume(self, setting, value, ctx):
        try:
            ...
            await ctx.send("`Error: Value must be a number`\nUsage: {}set {} 0-100".format(config.PREFIX, setting))
            return False
    ...
    async def vc_timeout(self, setting, value, ctx):
        ...
        if not config.ALLOW_VC_TIMEOUT_EDIT:
            await ctx.send("`Error: This value cannot be modified".format(config.PREFIX, setting))
